...
if [ $# -lt 2 ]; then
    ...
    ModWavDir=./wav/$Qry_type/$DB_type; mkdir -pv $ModWavDir
    ...
    thabs_db=1;
    thabs_qry=$(bc <<< "(0.25 * 2^15)/1");  # Accounted for short int, instead of float (0.25 -> 8192)
    threshold=110;
    TopN=2;

    # readmode = 1 : For mp3, can use for read wave too, but will be slower, can has memory leak
    # readmode = 2 : Directly read *.wav at 8kHz
    # readmode = 3 : Read *.wav at 16kHz, with a downsampler to 8kHz
    readmode=2;
    ...
    ## Audio/Spectrogram
        nfft=$nfft                      \t# fft size
        readmode=$readmode              \t# read mp3 or read wave (only for database now)
    ...
    ## Misc
        verbose=$verbose                \t# verbosity for hashing and querying
    "
fi
