...
/* ================================= Query ===================================================== */
    ...
    if (f_qry_wavscp.is_open()) {
        ...
        /* ================================================= */
            ...
            // computeFullFFT_double(numFrames, nfft, hop_length, (double*)buffer_8k, windowFunction, (double*)Spectrogram);
            computeFullFFT_double_fast(
                numFrames, nfft, hop_length,
                (double*)buffer_8k, windowFunction, (double*)Spectrogram,
        ...
        /* ================================================= */
            ...
            CandidateSet = searchLUT_v1(LUT, hash_DICT_qry);
            printf("CandidateSet.size() = %lu\n", CandidateSet.size());
            int shash_decision = 0, Max_shash_decision = 0, Max_num_matches=0;
            string Max_utt_DB;
            ...
            for (const auto &utt_DB : CandidateSet){
                ...
                // Perform matching
                shash_decision = shashmatching_v1(hash_DICT_db, hash_DICT_qry, num_matches, threshold_match, bins);
                if (verbose>0) {
                ...
                if ((shash_decision==1) && (num_matches>Max_num_matches)) {
                    Max_num_matches = num_matches;
                    Max_utt_DB = utt_DB;
                    Max_shash_decision = 1;
                }

            }
            // Found a match
            if (Max_shash_decision==1) {
                hash2txt(shashpath, hash_DICT_qry);
                printf("\t%s matched with %s , Max_num_matches=%i\n", utt_QRY.c_str(), Max_utt_DB.c_str(), Max_num_matches);
                f_qry2db << utt_QRY << " " << Max_utt_DB << " " << Max_num_matches << endl;
...
g++ -o ./bin/Shazam_querying_Full.out -Ofast -Wall -Wextra     -std=c++11 "Shazam_querying_Full.cpp" "fir_api.c"     -lavformat -lavcodec -lavutil -lswresample     $(pkg-config --libs --cflags sndfile)     $(pkg-config --libs --cflags fftw3) && echo "Finished compiling"

./bin/Shazam_querying_Full.out \
