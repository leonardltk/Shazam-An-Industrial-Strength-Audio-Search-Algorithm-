...
class Shazam_conf():
    def __init__(self):
        ## These are parameters for when the queries are clean.
        if True :
            ############ Shazam Constellation Point Details ############
            self.f_dim1, self.t_dim1=10,10
            self.base=1
            self.threshold_abs=1
        ...
        ## If the above parameters are not working, it is probably becasue your queries are noisy.
        # Try a more relaxed parameters, note that the hashing and matching time will increase, also False Negatives will increase.
        if True :
            ############ Shazam Constellation Point Details ############
            self.f_dim1, self.t_dim1=5,5 # This gives a smaller window to choose the peaks, hence producing more peaks in total
            self.base=20 # filter out more of the low frequency, if there are more noise.
            self.threshold_abs=.25
            ############ Shazam Hash Generation Details ############
            self.num_tgt_pts=5 # Increase more hash per anchor, as in noisy scenerio, you are likely to be hashing the noises
            self.delay_time_secs=0.27   # Apply the search in a closer region
            self.delta_time_secs=1.20   # Reduce the search in a smaller region
            self.delta_freq_Hz=365      # Reduce the search in a smaller region
            ############ Shazam Hash Clustering Details ############
            # If threshold_cluster is more than this amount, we classify them as within the same cluster
            # self.threshold_short=100 # Tested for noisy short audio of 20 secs
            self.threshold_short=200 # Tested for noisy short audio of 20 secs
            self.threshold_long=30*100 # Untested
            self.th=self.threshold_short
            self.cluster_verbose=False
    ...
    def __str__(self):
        ...
        print('\t\tthreshold_abs            :',self.threshold_abs)
