...
def write_audio(filename_out,x_in,sr,mode="soundfile"):
    ...
    if mode == "librosa":
        if np.max(np.abs(x_in))>1:
            print( f'{filename_out} is out of range [{np.min(x_in)},{np.max(x_in)}] , clipping ...' )
            x_in[x_in<-1]=-1
            x_in[x_in>1]=1
...
def display_audio(sig, title, 
    ...
    colour_to_set='white', curr_fig=None, kcoll=None, kcoll_list=None):

    if curr_fig and kcoll_list:
        ax = curr_fig.add_subplot(kcoll_list)
    elif curr_fig and kcoll:
...
def seconds_to_samples(secs, hop_length, n_fft, sr): return secs*sr

def Hz_to_freqidx(hz_in, num_freq_bins, sr):
    # num_freq_res = (sr/2)/num_freq_bins
    # int(hz_in / num_freq_res)
    return int(2*num_freq_bins*hz_in/sr)
def freqidx_to_Hz(freqidx, num_freq_bins, sr):
    return int((freqidx*sr) / (2*num_freq_bins))
...
def wav2LPS_speedtest(x_in, spect_det=None, mode="librosa",
    ...
    if mode == "librosa":
        ...
        """
        x_tmp = x_in+0.
        # print(f"x_stft = librosa.stft(y=x_tmp, n_fft={n_fft}, win_length={win_length}, hop_length={hop_length}, center={center_in}, window=_window)")
        x_stft = librosa.stft(y=x_tmp, n_fft=n_fft, win_length=win_length, hop_length=hop_length, center=center_in, window=_window)
