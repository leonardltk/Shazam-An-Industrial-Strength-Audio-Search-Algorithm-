#include "submodule/video_watermark_imo.h"

using namespace bigoai;

VideoWatermarkImoSubModule::VideoWatermarkImoSubModule() {}

bool VideoWatermarkImoSubModule::init(const SubModuleConfig &conf) {
...
bool VideoWatermarkImoSubModule::call_service(ContextPtr &ctx) {
    resp_.clear();
    multi_success_ = 0;
    ...
    req.set_id(id);
    req.set_post_id(id);
    req.set_isimage(false);
    auto video_size = ctx->normalization_msg().data().video_size();
    // LOG(INFO) << "video size: " << video_size;
    auto channel = channel_manager_.get_channel(url_);
    for (int i = 0; i < video_size; ++i) {
        success_ = false;
        VideoWatermarkImoResponse resp;
        if (ctx->normalization_msg().data().video(i).failed_module().size() != 0)
            continue;
        req.clear_video();
...
bool VideoWatermarkImoSubModule::post_process(ContextPtr &ctx) {
    ...
    for (unsigned int i = 0; i < resp_.size(); ++i) {
        ...
        if (res.xgb_score() >= 2 && res.cls_names_size()) {
            auto score = res.xgb_l1_score(res.xgb_score());
            result.add_scores(score);
            for (auto name : res.cls_names()){
        ...
        ctx->mutable_normalization_msg()->mutable_data()->mutable_video(i)->mutable_modelinfo()->insert(
...
RegisterClass(SubModule, VideoWatermarkImoSubModule);
