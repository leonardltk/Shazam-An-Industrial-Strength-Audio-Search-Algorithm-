#include "submodule/asr_v3.h"

using namespace bigoai;

AsrV3SubModule::AsrV3SubModule() {}


bool AsrV3SubModule::init(const SubModuleConfig &conf) {
...
bool AsrV3SubModule::call_keywords(ContextPtr ctx, std::string asr_text, keywords::Response &response) {
    response.Clear();
...
bool AsrV3SubModule::call_service(ContextPtr &ctx) {
    need_asr_ = false;
    ...
    if (cntl.Failed()) {
        LOG(ERROR) << "Fail to call asr_v3 service, " << cntl.ErrorText() << "[" << name() << "]"
...
bool AsrV3SubModule::post_process(ContextPtr &ctx) {
    if (!need_asr_) {
        return true;
    }
    ...
    LOG(INFO) << "AsrV3 resp: " << asr_debug << "; nlp resp: " << nlp_debug << "; keywords resp: " << keyword_debug
              << " [ID:" << ctx->traceid() << "] ";
    ...
    if (word_resp_.nlp().items_size() != 0  && word_resp_.nlp().items(0).label() == 1) {
        add_hit_num(1);
...
RegisterClass(SubModule, AsrV3SubModule);
