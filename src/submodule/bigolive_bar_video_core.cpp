#include "submodule/bigolive_bar_video_core.h"

using namespace bigoai;

BigoliveBarVideoCoreSubModule::BigoliveBarVideoCoreSubModule() {}

bool BigoliveBarVideoCoreSubModule::init(const SubModuleConfig &conf) {
...
bool BigoliveBarVideoCoreSubModule::call_service(ContextPtr &ctx) {
    resp_.Clear();
...
bool BigoliveBarVideoCoreSubModule::call_service_batch(std::vector<ContextPtr> &ctx) {
    batch_resp_.clear();
    for (auto &c : ctx) {
        call_service(c);
...
bool BigoliveBarVideoCoreSubModule::post_process(ContextPtr &ctx, BigoMLServiceVideoCoreResponse &resp) {
    if (context_is_download_failed(ctx) || resp_.results().empty()) {
        for (auto &model_name : model_names_) {
            set_context_detailmlresult_modelinfo_with_type(ctx, model_name, MODEL_RESULT_FAIL, "", "video");
        }
        return true;
    }
    ...
    for (int i = 0; i < resp_.results_size(); ++i) {
        ...
        set_context_detailmlresult_modelinfo_with_type(ctx, resp_.results(i).model_name(), res, info_str, "video");
    }
...
bool BigoliveBarVideoCoreSubModule::post_process(ContextPtr &ctx) { return post_process(ctx, resp_); }

bool BigoliveBarVideoCoreSubModule::post_process_batch(std::vector<ContextPtr> &ctxs) {
...
RegisterClass(SubModule, BigoliveBarVideoCoreSubModule);
